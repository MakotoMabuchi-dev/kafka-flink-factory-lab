...
Superset も `superset-local/` の local stack として起動でき、
Trino 接続と lab 用 dashboard をプリセットした状態まで自動で作れます。

この README では、背景説明より先に、
すぐ試せる導線、`lab_console` の全コマンド説明、永続データの扱い、
Iceberg と Trino と Superset までの確認手順を追えるようにしています。
...
Superset は `admin / admin` でログインでき、
`trino_iceberg_lab` という Trino 接続と
`Factory Lab Overview` dashboard があらかじめ入ります。
`8088` が使用中なら `8089` 以降の空き port に自動で退避します。
...
- `lab_console` は開始時に既存の Trino と Superset も停止し、`flink-test/minio-data` `flink-test/iceberg-rest-data` `superset-local/data` を消します
- 起動後は Trino と Superset も自動で立ち上がるので、`superset-open` ですぐに prebuilt dashboard を開けます
- `exit` 時には Trino と Superset を含む起動済み環境を停止し、同じ保存データを削除します
...
- Superset
  - `superset-local/` の local stack で起動する
  - 起動時に `trino_iceberg_lab` 接続と `Factory Lab Overview` dashboard を自動で作る
  - `Superset -> Trino -> Iceberg` の順で参照する
...
- `scripts/lab_cli.py superset-start` `superset-open` などで Superset を扱え、`superset-open` は prebuilt dashboard を開く
- `scripts/lab_cli.py console` で対話コンソールを起動でき、Trino と Superset も自動起動する
- `trino-test/` に Trino Compose と Iceberg catalog 設定がある
- `superset-local/` に Superset local stack がある

重要:

- `scripts/lab_cli.py console` は開始時と終了時に `flink-test/minio-data` `flink-test/iceberg-rest-data` `superset-local/data` を削除する
...
- `scripts/lab_cli.py`
  - 共通 CLI
  - `start` `stop` `status` `watch-topic` `apply-sql` `read-iceberg` `inspect-iceberg` `trino-start` `trino-read` `trino-query` `superset-start` `superset-open` `console` などをまとめて扱う
- `lab_console.sh`
...
- `superset-local/docker-compose.yml`
  - Superset local stack の Compose 定義
- `superset-local/assets/`
  - 接続済み database / dataset / chart / dashboard を import する bundle 雛形
- `superset-local/Dockerfile`
  - `apache/superset:6.0.0` に `trino` driver を追加する
- `superset-local/scripts/start_lab_superset.sh`
  - Superset 初期化、Trino 接続の upsert、bundle import を行う起動スクリプト
- `superset-local/scripts/prepare_lab_bundle.py`
  - 既存 database UUID に合わせて import bundle をレンダリングする
- `superset-local/pythonpath/superset_config.py`
  - Superset metadata と cache の local 設定
- `superset-local/README.md`
  - Superset local stack の最小メモ
...
- 既存の Kafka / Flink / MinIO / Iceberg REST / Trino / Superset を一度停止する
- `flink-test/minio-data` `flink-test/iceberg-rest-data` `superset-local/data` `.lab-runtime` を削除する
- stack を起動する
- Trino を起動する
- Superset を起動する
- producer の起動を試みる
...
- 起動した Kafka / Flink / MinIO / Iceberg REST / Trino / Superset を停止する
- `flink-test/minio-data` `flink-test/iceberg-rest-data` `superset-local/data` `.lab-venv` `.lab-runtime` を削除する
...
- `help`
  - 使えるコマンド一覧を表示する
- `status`
  - Docker コンテナ一覧、Flink jobs、producer の稼働状態、Trino と Superset の ready 状態をまとめて表示する
- `topics`
  - Kafka topic 一覧を表示する
- `watch <topic> [count]`
  - 指定 topic を先頭から読む
...
- `superset-status`
  - Superset が `STOPPED` `STARTING` `RUNNING` のどれかを表示する
  - `RUNNING` では URL、ログイン情報、DB 接続名、既定 dashboard URL も確認できる
- `superset-start`
  - `superset-local/docker-compose.yml` を使って Superset を起動する
  - `admin / admin` ユーザー、`trino_iceberg_lab` 接続、`Factory Lab Overview` dashboard を自動で揃える
- `superset-stop`
  - Superset コンテナを停止する
- `superset-log`
  - Superset コンテナの最新ログを表示する
- `superset-open`
  - ブラウザで `Factory Lab Overview` dashboard を直接開く
  - `8088` が埋まっている環境では実際の公開 port を自動検出して開く
  - 端末にはログイン情報も表示する
- `exit`
...
この repo には `superset-local/` が含まれており、
Superset を local stack として起動できます。
起動時に次が自動で揃います。

- Superset UI: `http://localhost:8088`
  - ただし `8088` が使用中なら `8089` 以降へ自動で退避する
- login: `admin / admin`
- database connection: `trino_iceberg_lab`
- SQLAlchemy URI: `trino://trino@trino:8080/iceberg`
- dashboard: `Factory Lab Overview`
- dashboard URL: `/superset/dashboard/factory-lab-overview/`

### 1. 先に Kafka / Flink / Trino 側を起動する
...
内部では `superset-local/docker-compose.yml` を使って `apache/superset:6.0.0` ベースのイメージを build し、
`superset-local/scripts/start_lab_superset.sh` で初期化と Trino 接続登録を行います。
...
ブラウザ起動コマンドが使えない環境では、手動で `http://localhost:8088` を開きます。
`superset-start` 実行時に別 port へ退避した場合は、その URL が端末に表示されます。

### 4. UI 上で確認する

`Settings -> Data -> Database Connections` に `trino_iceberg_lab` が見えていれば、
接続設定までは完了しています。

`Dashboards` に `Factory Lab Overview` が見えていれば、
dashboard の import まで完了しています。
...
### Superset が `STARTING` のまま進まない

- 初回は `apache/superset:6.0.0` の pull と image build に時間がかかる
- `python3 scripts/lab_cli.py superset-log` で `Applying Superset metadata migrations` の先まで進んでいるか確認する
- 失敗したら `python3 scripts/lab_cli.py superset-stop` のあとに `python3 scripts/lab_cli.py superset-start` を再実行する

### Superset に `trino_iceberg_lab` が見えない

- `python3 scripts/lab_cli.py superset-log` に `Configuring Trino connection trino_iceberg_lab` が出ているか確認する
- Trino 側が先に起動しているか `python3 scripts/lab_cli.py trino-status` で確認する
- `superset-local/data/` を消して作り直したい場合は、`lab_console` を使うか、`python3 scripts/lab_cli.py superset-stop` のあとで `superset-local/data/` を削除して再起動する
...
- このリポジトリには Trino の学習用設定と Superset local stack が含まれる
