...
`trino-test/` に手動学習用の Trino 設定も含んでいます。
...
- `lab_console` は開始時に `flink-test/minio-data` と `flink-test/iceberg-rest-data` を消します
- `exit` 時にも同じ保存データを削除します
...
- `scripts/lab_cli.py console` で対話コンソールを起動できる
- `trino-test/` に Trino の手動学習用 Compose と Iceberg catalog 設定がある
...
- `scripts/lab_cli.py`
  - 共通 CLI
  - `start` `stop` `status` `watch-topic` `apply-sql` `read-iceberg` `inspect-iceberg` `console` などをまとめて扱う
- `lab_console.sh`
...
- 既存の Kafka / Flink / MinIO / Iceberg REST を一度停止する
...
- `help`
  - 使えるコマンド一覧を表示する
- `status`
  - Docker コンテナ一覧、Flink jobs、producer の稼働状態をまとめて表示する
- `topics`
  - Kafka topic 一覧を表示する
- `watch <topic> [count]`
  - 指定 topic を先頭から読む
...
Iceberg REST Catalog と MinIO を手動で読む練習ができます。
...
この Compose は Trino を `stream-shared` network に参加させ、
`etc/catalog/iceberg.properties` で Iceberg REST Catalog と MinIO を参照します。
...
```bash
python3 scripts/lab_cli.py inspect-iceberg
docker exec -it trino trino --execute 'SELECT count(*) FROM iceberg.lab.product_summary_iceberg'
```
