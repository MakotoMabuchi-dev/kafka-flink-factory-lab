...
`trino_iceberg_lab` という Trino 接続があらかじめ入ります。
...
- 起動後は Trino と Superset も自動で立ち上がるので、`superset-open` をすぐ使えます
...
- Superset
  ...
  - 起動時に `trino_iceberg_lab` 接続を自動で作る
...
- `scripts/lab_cli.py superset-start` `superset-open` などで Superset を扱える
...
- `superset-local/scripts/start_lab_superset.sh`
  - Superset 初期化と Trino 接続の upsert を行う起動スクリプト
...
- `superset-status`
  ...
  - `RUNNING` では URL、ログイン情報、DB 接続名も確認できる
- `superset-start`
  - `superset-local/docker-compose.yml` を使って Superset を起動する
  - `admin / admin` ユーザーと `trino_iceberg_lab` 接続を自動で揃える
- `superset-stop`
  - Superset コンテナを停止する
- `superset-log`
