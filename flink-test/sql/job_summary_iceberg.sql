...
--   製品シリアル単位の summary を作成し、Iceberg に保存する。
--
-- Do:
--   - process-events から工程区間を作る
--   - 工程1に sensor-a、工程2に sensor-b を紐付ける
--   - 1製品1行の product_summary を作る
--   - 最終結果を Iceberg table に保存する
--
-- Don't:
--   - Superset 用の可視化設定までは行わない
--
-- Design Policy:
--   - 既存の job_summary.sql を土台にする
--   - Kafka source / 中間 VIEW は default catalog 上に作る
--   - Iceberg sink だけを lakehouse catalog に作る
-- ============================================
...
CREATE TABLE sensor_a_raw (
    ...
    event_time AS TO_TIMESTAMP(SUBSTRING(REPLACE(event_time_str, 'T', ' '), 1, 23)),
...
CREATE TABLE sensor_b_raw (
    ...
    event_time AS TO_TIMESTAMP(SUBSTRING(REPLACE(ts_str, 'T', ' '), 1, 23)),
    WATERMARK FOR event_time AS event_time - INTERVAL '1' SECOND
) WITH (
    'connector' = 'kafka',
    'topic' = 'sensor-b',
...
DROP TABLE IF EXISTS lakehouse.lab.product_summary_iceberg;

CREATE TABLE lakehouse.lab.product_summary_iceberg (
    serial_no STRING,
    equipment_id STRING,
...
INSERT INTO lakehouse.lab.product_summary_iceberg
SELECT
    serial_no,
    equipment_id,
